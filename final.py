...
for heading in headings + parameters:                           # work needs to be done here
...
for frame in range(frame_count):
    ...
    for _ in range(total_parameters):                                          # need work done here
        if index in range(52, 181) or index in range(184, 313) or index in range(316, 389) or index in range(392, 441) or index in range(450, 699):
            parameter_data = parameters_data[index:index + 8]
            parameter_value = bytes_to_double(parameter_data)
        elif index in range(181, 184) or index in range(313, 316):
            index += 3
            continue
        elif index in range(441, 450):
            index += 9
            continue
        elif index in range(699, 710):
            index += 11
            continue
        else:
            parameter_data = parameters_data[index:index + 8]
